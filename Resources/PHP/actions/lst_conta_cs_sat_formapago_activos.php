<?PHP
  $query_consultaFormapago = "SELECT * FROM conta_cs_sat_formapago WHERE s_activo = 'S' ORDER BY s_concepto";
  ...
  if ($rows_consultaFormapago > 0) {
      ...
      while ($row_consultaFormapago = $rslt_consultaFormapago->fetch_assoc()) {
        ...
        $consultaFormapago .= '<option value="'.$id_Formapago.'+'.$concepto.'">'.$concepto.' --- '.$id_Formapago.'</option>';
      }
  }
