...
$query_conTarifaPOCMEgeneral = "SELECT fk_id_conceptoCta,s_concepto_eng,s_concepto_esp,n_cantidad,n_importe
...
if (!($stmt_conTarifaPOCMEgeneral->execute())) {
  $system_callback['code'] = "500";
  $system_callback['message'] = "Error during query execution [$stmt_conTarifaPOCMEgeneral->errno]: $stmt_conTarifaPOCMEgeneral->error";
  exit_script($system_callback);
}
...
if ($rslt_conTarifaPOCMEgeneral->num_rows > 0) {
  ...
  while ($row_conTarifaPOCMEgeneral = $rslt_conTarifaPOCMEgeneral->fetch_assoc()) {
    $fk_id_conceptoCta = $row_conTarifaPOCMEgeneral[fk_id_conceptoCta];
    $s_concepto_eng = trim(utf8_encode($row_conTarifaPOCMEgeneral[s_concepto_eng]));
    $s_concepto_esp = trim(utf8_encode($row_conTarifaPOCMEgeneral[s_concepto_esp]));
    $n_cantidad = $row_conTarifaPOCMEgeneral[n_cantidad];
    $n_importe = number_format($row_conTarifaPOCMEgeneral[n_importe],2);

    $tarifaPOCMEgeneral .= "<option value='$fk_id_conceptoCta+$s_concepto_eng+$n_cantidad+$n_importe+$s_concepto_esp'>$s_concepto_esp $n_importe</option>";
  }
}
?>
