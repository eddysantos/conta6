...
if( $id_empleado == 'Todas' && $id_nomina > 0 && $id_aduana > 0 ){ $consulta = 'n_anio ='.$anio.' AND fk_id_aduana ='.$id_aduana.' AND n_semana ='.$id_nomina; }
	...
	$txtSQL_percepciones = "SELECT SELECT n_ordenReporte,s_conceptoReporte,s_clasificacion
							FROM conta_cs_sat_tipoPercepcion_ctaMst
							WHERE fk_id_regimen = '$id_regimen' AND n_ordenReporte > 0
							ORDER BY n_ordenReporte";

	$txtSQL_otrosPagos = "SELECT SELECT n_ordenReporte,s_conceptoReporte,s_clasificacion
						   FROM conta_cs_sat_tipoOtroPago_ctaMst
						   WHERE fk_id_regimen = '$id_regimen' AND n_ordenReporte > 0
						   ORDER BY n_ordenReporte";

	$txtSQL_deducciones = "SELECT SELECT n_ordenReporte,s_conceptoReporte,s_clasificacion
...
<table border="0" width="100%" cellspacing="1" style="font-family: Trebuchet MS;">
		...
		<tr style="font-size: 10pt;" align=center>
			...
			<td width="20%" colspan="2"><b>Fecha Inicio: <?php echo  date_format(date_create($sql_consultaNominaFecha['fechaInicio']),"d-m-Y"); ?></b></td>
			<td width="20%" colspan="2"><b>Fecha Final:<?php echo  date_format(date_create($sql_consultaNominaFecha['FechaFinal']),"d-m-Y"); ?></b></td>
...
<table border="0" width="100%" cellspacing="1" style="font-family: Trebuchet MS;">
		<tr>
			<td width="100%" colspan="10" bgcolor="#000000" align="center" style="color:#FFFFFF">Empleados Bajo el Régimen de Sueldos y Salarios</td>
		</tr>
</table>
...
<?php
	...
	while( $oRst_consultaConceptoOP = mysqli_fetch_array($sql_consultaConceptoOP)){
		$ordenReporteOP = $oRst_consultaConceptoOP['s_ordenReporte'];
		$rowspanOP = "rowspan=2";;
		echo '<td style="background-color:#DCDCDC; color:#000000" '.$rowspanOP.'>'.$oRst_consultaConceptoOP['s_conceptoReporte'].'</td>';
	}
?>
...
<?php
	...
	while($oRst_consultaConceptoP = mysqli_fetch_array($sql_consultaConceptoD)){
		$ordenReporteD = $oRst_consultaConceptoP['s_ordenReporte'];
		if( $ordenReporteD == 7 ){ $rowspanD = "colspan=3"; }else{ $rowspanD = "rowspan=2"; }
		echo '<td style="background-color:#DCDCDC; color:#000000" '.$rowspanD.'>'.$oRst_consultaConceptoP['s_conceptoReporte'].'</td>';}
?>
...
?>
	...
	<?php
		...
		while($oRst_consultaOrdenP = mysqli_fetch_array($sql_consultaOrdenP)){
			$ordenP = $oRst_consultaOrdenP['s_ordenReporte'];
			...
			$sql_consultaDetalleP=mysqli_query($db,"SELECT sum(n_importeGravado) as importeGravado,sum(n_importeExento) as importeExento,sum(n_importePagado) as importePagado,sum(n_dias_horasExtra) as dias_horasExtra,sum(n_horasExtra) as horasExtra
													FROM conta_t_nom_captura_det
													where fk_id_docNomina = $fk_id_docNomina and (s_clasificacion = 'percepcion' or s_clasificacion = 'horasExtras' or s_clasificacion = 'separacionIndemnizacion') and s_ordenReporte = $ordenP");
		...
		while($oRst_consultaOrdenOP = mysqli_fetch_array($sql_consultaOrdenOP)){
			$ordenOP = $oRst_consultaOrdenOP['s_ordenReporte'];

			$sql_consultaDetalleOP=mysqli_query($db,"SELECT *
													FROM conta_t_nom_captura_det
													where fk_id_docNomina = $id_docNomina and s_clasificacion = 'otrosPagos' and s_ordenReporte = $ordenOP");
	...
	<?php
		...
		while( $oRst_consultaOrdenD = mysqli_fetch_array($sql_consultaOrdenD)){
			$ordenD = $oRst_consultaOrdenD['s_ordenReporte'];

			$sql_consultaDetalleD=mysqli_query($db,"SELECT *
													FROM conta_t_nom_captura_det
													where fk_id_docNomina = $id_docNomina and (s_clasificacion = 'deduccion' or s_clasificacion = 'desctoDespTotal') and s_ordenReporte = $ordenD");
			...
			if( $totalRegistros > 0 ){
				while($oRst_consultaDetalleD = mysqli_fetch_array($sql_consultaDetalleD)){
					if( $ordenD == 7 ){
						...
						echo '<td>'.$base.'</td><td>'.$porcentaje.'</td><td align="right">'.$importe.'</td>';}else{
						$importe = $oRst_consultaDetalleD['n_importeGravado'] + $oRst_consultaDetalleD['n_importeExento'];
						$importe = number_format($importe,2,'.',',');
						echo '<td align="right">'.$importe.'</td>';}}}else{
				if( $ordenD == 7 ){
...
<?php
		...
		// 												FROM conta_t_nom_captura_det
		// 												WHERE fk_id_docNomina IN(SELECT fk_id_docNomina
		// 																		FROM TBL_NOM_nominaCFDI
		// 																		WHERE id_regimen = $id_regimen AND $consulta AND selloSATcancela is null) and tipoElemento = 'totales'"));
    ...
    //                                     c.s_selloSATcancela is null and a.s_tipoElemento  ='totales' and b.fk_id_regimen = '$id_regimen' and $consulta "));
		...
		// 	$ordenTotalesP = $oRst_consultaOrdenPtotales['s_ordenReporte'];
		// 	if( $ordenTotalesP == 5 or $ordenTotalesP == 6){ $txtSumTotales = "importePagado"; }else{ $txtSumTotales = "importeGRAVADO + importeEXENTO"; }
		// 	$sql_consultaPtotales = mysqli_query($db,"SELECT SUM($txtSumTotales) AS TOTAL
		// 													FROM conta_t_nom_captura_det
		// 													WHERE fk_id_docNomina IN(SELECT fk_id_docNomina
		// 																		FROM TBL_NOM_nominaCFDI
		// 																		WHERE id_regimen = $id_regimen AND $consulta AND selloSATcancela is null) AND s_ordenReporte = $ordenTotalesP and (s_clasificacion = 'percepcion' or s_clasificacion = 'horasExtras')");
...
<?php
		...
		// 	$ordenTotalesP = $oRst_consultaOrdenOtrosPtotales['s_ordenReporte'];
		// 	$txtSumTotales = "importeGRAVADO + importeEXENTO";
		// 	$sql_consultaOPtotales = mysqli_query($db,"SELECT SUM($txtSumTotales) AS TOTAL
		// 													FROM conta_t_nom_captura_det
		// 													WHERE fk_id_docNomina IN(SELECT fk_id_docNomina
		// 																		FROM TBL_NOM_nominaCFDI
		// 																		WHERE id_regimen = $id_regimen AND $consulta AND selloSATcancela is null) AND s_ordenReporte = $ordenTotalesP and (s_clasificacion = 'otrosPagos')");
...
<?php
		...
		// 	$ordenTotalesD = $oRst_consultaOrdenDtotales['s_ordenReporte'];
		// 	if( $ordenTotalesD == 5 or $ordenTotalesD == 6){ $txtSumTotales = "descuento"; }else{ $txtSumTotales = "importeGRAVADO + importeEXENTO"; }
		// 		$sql_consultaDtotales=mysqli_query($db,"SELECT SUM($txtSumTotales) AS TOTAL
		// 													FROM conta_t_nom_captura_det
		// 													WHERE fk_id_docNomina IN(SELECT fk_id_docNomina
		// 																		FROM TBL_NOM_nominaCFDI
		// 																		WHERE id_regimen = $id_regimen AND $consulta AND selloSATcancela is null) AND s_ordenReporte = $ordenTotalesD and (s_clasificacion = 'deduccion' or s_clasificacion = 'desctoDespTotal')");
