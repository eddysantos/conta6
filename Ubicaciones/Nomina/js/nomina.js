$(document).ready(function(){
  ...
  $('#generarNuevoDocumento').click(function(){
    ...
    if ($('#semana').val() == 0 || $('#anio').val() == 0 || $('#nom_regimen').val() == 0) {
      alertify.error('Regimen, semana o año no pueden estas vacias');
      return false;
    }else if ($('#opcionestipo').val() == 0) {
      alertify.error('Selecciona el tipo de nomina');
      return false;
    }else if ($('#opcionesdescNom').val() == 0) {
      alertify.error('Selecciona el documento de nomina');
      return false;
    }else if ($('#empleado').val() == "") {
      alertify.error('Selecciona el nombre de empleado');
      return false;
    }else {
        ...
        success: 	function(r){
          ...
          if (r.code == 1) {
      			alertify.alert('Documento generado correctamente', function(){
      				...
      				if( idRegimen == '02'){
      					document.location.replace('/Ubicaciones/Nomina/SueldosySalarios/consultar_Nomina.php');
      				}
      				if( idRegimen == '09'){
      					document.location.replace('/Ubicaciones/Nomina/Honorarios/consultar_Nomina.php');
      				}
    ...
    $.ajax({
      ...
      success: 	function(r){
        ...
        if (r.code == 1) {
          ...
          // });
          swal("Modificado!", "Se actualizo correctamente.", "success");
        } else {
