<?php
  ...
  $nomBan = trim($_POST['nomBan']);
	$system_callback = [];
	$data = $_POST;
  $query_INSERT = "INSERT INTO conta_cs_bancos_beneficiarios (fk_id_benef,fk_id_banco,s_nomBanExt,s_cta_banco,s_usuario_alta) VALUES (?,?,?,?,?)";
  ...
  $stmt_INSERT->bind_param('sssss',$id_ben,$banco,$nomBan,$cuenta,$usuario);
  if (!($stmt_INSERT)) {
    $system_callback['code'] = "500";
  ...
  $descripcion = "Se agrego bco/cta al Beneficiario: $id_ben Banco: $banco Nombre: $nomBan Cuenta: $cuenta";
  $clave = 'benef';
  $folio = $id_ben;
