<?php
#http://localhost:88/Ubicaciones/Contabilidad/Reportes/contabilidad/balanzaComprobacion_oficina.php?numreporte=4&Oficina=240&Fecha_Inicial=2018-05-23&Fecha_Final=2020-08-31
...
if (  $rslt_permisosReportes->num_rows > 0 ) {
  $Oficina = $aduana; # el contador solo puede ver informacion de su oficina
  require $root . '/Ubicaciones/Contabilidad/Reportes/actions/consultaPermisosReportesOficina.php';
  if ($rslt_permisosReportesOficina->num_rows > 0) {

    if( $Oficina == '470' ){ $nomOficina = 'Aeropuerto'; }
  	if( $Oficina == '240' ){ $nomOficina = 'Nuevo Laredo'; }
  	if( $Oficina == '160' ){ $nomOficina = 'Manzanillo'; }
  	if( $Oficina == '430' ){ $nomOficina = 'Veracruz'; }

    $fechaActual = date("YmdHis");

    $periodo = 'comprobacion';
    require $root . '/Ubicaciones/Contabilidad/Reportes/actions/sp_balanza_oficina_activa.php';

    $tblGastos = 'temconta_BCofac_gastos_'.$fechaActual;
    $tblGastos_idx = 'idx_BCofac_gastos_'.$fechaActual;
    mysqli_query($db,"CREATE TABLE $tblGastos(
                        ...
                        where fk_gastoAduana > 0 and fk_id_cuenta = '0168-00005' and d_fecha between '$Fecha_Inicial' and '$Fecha_Final' and fk_gastoAduana = $Oficina
    ...
    ?>
      ...
      <html lang="es">
          ...
          <!-- fany, por cuestion de formato para los miles se pone una coma, no sé si hay forma de darle ese formato a la celda con CSS y asi evitaremos poner number_format y redondear a 2 decimales -->
      		<tr>
    ...
    <?php
    $lst_tablas = $tblBalanza.','.$tblGastos;
