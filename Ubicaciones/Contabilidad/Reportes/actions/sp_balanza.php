...
#contabilidad > balanza comprobacion / contabilidadElectronica > balanza comprobacion XML
	if( $periodo == 'comprobacion' ){
		$tblBalanza = 'temconta_BC_'.$fechaActual;
		$tblSaldos = 'temconta_BC_sal_'.$fechaActual;
		$tblMov = 'temconta_BC_mov_'.$fechaActual;
		$tblSalIni = 'temconta_BC_salini_'.$fechaActual;
		$tblCtasMes = 'temconta_BC_ctasmes_'.$fechaActual;
		$tblBal1 = 'temconta_BC_balanza1_'.$fechaActual;
 	}

 #-- Calculamos los cargos y abonos para saldos iniciales de todas las cuentas
  mysqli_query($db,"CREATE TABLE $tblSaldos
  select fk_id_cuenta, SUM(n_cargo) as cargos, SUM(n_abono) as abonos
  from conta_t_polizas_det
  where d_fecha < '$Fecha_Inicial'
  ...
  select fk_id_cuenta,
      case
        ...
        when left(fk_id_cuenta, 2) = '05' then (cargos - abonos)
				-- when left(fk_id_cuenta, 2) = '06' then (cargos - abonos)
				-- _fany_
