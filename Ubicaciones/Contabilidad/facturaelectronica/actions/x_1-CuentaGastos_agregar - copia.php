...
$sql = "INSERT INTO tabla_detalle (campo1, campo2, campo3) VALUES(?,?,?)";

$inset_stmt = $db->perpare($sql);



foreach ($pocmes as $pocme) {
  $descripcion_esp = $pocme['descripcion_esp'];
  $descripcion_ing = $pocme['descripcion_ing'];
  $importe = $pocme['importe'];

  $insert_stmt->bind_param('sss', $descripcion_esp, $descripcion_ingles, $importe);
  $insert_stmt->execute();
}
