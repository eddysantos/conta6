<?PHP
  ...
  //guardando cheque
  $queryInsert = "INSERT INTO conta_t_cheques_mst (fk_id_cuentaMST,d_fecha,s_tipoOrdenante,fk_idOrd,s_nomOrd,n_valor,fk_usuario,fk_id_aduana,s_concepto )
                  VALUES(?,?,?,?,?,?,?,?,?)";
  ...
  $stmtInsert->bind_param('sssssssss',$cuenta,$fechaDoc,$opcion,$idOrd,$nomOrd,$importe,$usuario,$aduana,$concepto);
  if (!($stmtInsert)) {
  	$system_callback['code'] = "500";
  ...
  $descripcion = "Se Genero el Cheque: $nFolio Concepto: $concepto Fecha: $fechaDoc Valor: $importe Cuenta:$cuenta ExpedidoA: $opcion $idOrd $nomOrd";
  $clave = 'cheques';
  $folio = $nFolio;
